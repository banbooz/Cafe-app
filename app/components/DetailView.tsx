  ...
  return (
    <>
      <main className="min-h-screen bg-[#f7f7f5] px-4 pb-32 pt-4 text-[#1d2528]">
        ...
        <section className="mt-2 overflow-hidden rounded-[2rem] bg-white shadow-[0_24px_60px_rgba(29,37,40,0.14)] ring-1 ring-black/5">
          <div className="grid h-80 grid-cols-[1.2fr_0.8fr] gap-1 bg-white p-1">
            <div className="relative rounded-[1.75rem] bg-cover bg-center" style={{ backgroundImage: `url(${item.image})` }}>
              {unavailable && <UnavailableOverlay />}
            </div>
            <div className="grid gap-1">
              <div className="rounded-[1.4rem] bg-cover bg-center" style={{ backgroundImage: `url(${item.image})` }} />
              <div className="relative rounded-[1.4rem] bg-cover bg-center" style={{ backgroundImage: `url(${item.image})` }}>
          ...
          <div className="p-5">
            ...
            <div className="mt-5 grid gap-3">
              <Info title="Prep time" text={item.prep} />
              <Info title="Allergens" text={item.allergens.join(", ")} />
            </div>
          </div>
        </section>
      </main>

      <section className="fixed bottom-0 left-1/2 z-50 w-full max-w-[430px] -translate-x-1/2 px-4 pb-[calc(0.8rem+env(safe-area-inset-bottom))] pt-3 sm:bottom-5">
        {unavailable ? (
          ...
          <div className="flex min-h-16 items-center gap-2 rounded-full bg-[#263238]/95 p-2 text-white shadow-[0_20px_50px_rgba(29,37,40,0.32)] backdrop-blur-xl">
            ...
            <button onClick={() => add(item.id)} className="grid h-12 w-12 place-items-center rounded-full bg-[#ff385c] text-lg font-black shadow-lg">+</button>
          </div>
        ) : (
          <button onClick={() => add(item.id)} className="min-h-16 w-full rounded-full bg-[#ff385c] px-5 text-sm font-black text-white shadow-[0_20px_50px_rgba(255,56,92,0.28)]">Add to basket</button>
        )}
      </section>
    </>
  );
