...
const staffSecondaryButton = "bg-slate-100 text-slate-700 ring-1 ring-slate-200";
const staffWhiteButton = "bg-white text-slate-700 ring-1 ring-slate-200";
    ...
    <section className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-slate-200">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <button onClick={() => setOpen((current) => !current)} className="flex flex-1 items-start justify-between gap-4 text-left">
          ...
          <span className={`shrink-0 whitespace-nowrap rounded-2xl px-4 py-3 text-xs font-black ${staffSecondaryButton}`}>{open ? "Hide" : "Show"} menu</span>
        </button>
        <div className="flex shrink-0 flex-wrap gap-2">
          <a href={otherHref} className={`rounded-2xl px-4 py-3 text-xs font-black ${staffSecondaryButton}`}>{otherLabel}</a>
          <button onClick={resetAvailability} className={`rounded-2xl px-4 py-3 text-xs font-black ${staffSecondaryButton}`}>Reset availability</button>
          <button onClick={resetMenuSettings} className={`rounded-2xl px-4 py-3 text-xs font-black ${staffSecondaryButton}`}>Reset text</button>
        </div>
      </div>
    ...
    <article className="overflow-hidden rounded-3xl bg-slate-50 p-3 ring-1 ring-slate-200">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => setItemAvailability(item.id, !available)} aria-pressed={available} className={`flex min-h-10 min-w-0 items-center justify-center rounded-2xl px-2 text-center text-xs font-black leading-tight ${staffPrimaryButton}`}>{available ? "Set unavailable" : "Set available"}</button><button onClick={() => toggleEditing(item.id, editing)} className={`flex min-h-10 min-w-0 items-center justify-center rounded-2xl px-2 text-xs font-black ${staffWhiteButton}`}>{editing ? "Close" : "Edit"}</button></div>
        </div>
      </div>
...
function ProductEditPanel({ item, categoryOptions, activeTab, setActiveTab, updateItemSettings, handleImageUpload, setItemInactive }: Pick<StaffItemCardProps, "item" | "categoryOptions" | "activeTab" | "setActiveTab" | "updateItemSettings" | "handleImageUpload" | "setItemInactive">) {
  return (
    <div className="mt-3 rounded-2xl bg-white p-3 ring-1 ring-slate-200">
      ...
      {activeTab === "dietary" && <div className="mt-3 space-y-3"><div className="flex flex-wrap gap-2"><button onClick={() => updateItemSettings(item.id, { vegetarian: !item.vegetarian })} className={item.vegetarian ? `min-h-11 rounded-full px-4 py-2 text-xs font-black ${staffPrimaryButton}` : `min-h-11 rounded-full px-4 py-2 text-xs font-black ${staffSecondaryButton}`}>V</button><button onClick={() => updateItemSettings(item.id, { vegan: !item.vegan })} className={item.vegan ? `min-h-11 rounded-full px-4 py-2 text-xs font-black ${staffPrimaryButton}` : `min-h-11 rounded-full px-4 py-2 text-xs font-black ${staffSecondaryButton}`}>VG</button></div><label className="block"><span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Allergens</span><input value={item.allergens.join(", ")} onChange={(event) => updateItemSettings(item.id, { allergens: cleanAllergenList(event.target.value) })} className="mt-2 w-full min-w-0 rounded-xl bg-slate-50 p-3 text-xs font-bold outline-none ring-1 ring-slate-200" /></label></div>}
      ...
      {activeTab === "off" && <div className="mt-3 rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-200"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-700">Move to inactive</p><p className="mt-2 text-xs font-bold leading-5 text-slate-600">This item leaves the live menu. Old orders keep the item name and it can be added back later.</p><button onClick={() => setItemInactive(item.id, item.name)} className={`mt-3 min-h-11 w-full rounded-2xl px-4 text-xs font-black ${staffPrimaryButton}`}>Move to Inactive Items</button></div>}
    </div>
  );
}
...
function InactiveItemsView({ items, setItemActive }: { items: MenuItem[]; setItemActive: (id: number) => void }) {
  ...
  return <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-3">{items.map((item) => <article key={item.id} className="rounded-3xl bg-slate-50 p-3 ring-1 ring-slate-200"><div className="flex gap-3"><div className="h-16 w-16 shrink-0 rounded-2xl bg-cover bg-center opacity-70" style={{ backgroundImage: `url(${item.image})` }} /><div className="min-w-0 flex-1"><p className="truncate text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">{item.category}</p><h3 className="truncate font-black text-slate-950">{item.name}</h3><p className="mt-1 truncate text-xs font-bold text-slate-500">{money(item.price)} · {item.prep}</p></div></div><button onClick={() => setItemActive(item.id)} className={`mt-3 min-h-11 w-full rounded-2xl px-4 text-xs font-black ${staffPrimaryButton}`}>Add Back to Menu</button></article>)}</div>;
}
