...
export default function StaffMenuControls({ section, compact = false }: Props) {
  ...
  const catalogue = useMenuCatalogue(settings);
  const { staffItems, hiddenIds, visibleItems, addStaffProduct, hideStaffProduct } = catalogue;
  const inactiveSet = new Set(hiddenIds);
  const inactiveItems = [...menuItems, ...staffItems].filter((item) => inactiveSet.has(item.id)).map((item) => applyMenuSettings(item, settings));
  ...
  function setItemInactive(id: number, name: string) {
    const confirmed = window.confirm(`${name} will move to Inactive Items. It will leave the live customer menu, while old orders and analytics keep their history. You can add it back later.`);
    if (!confirmed) return;
    hideStaffProduct(id);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <button onClick={() => setOpen((current) => !current)} className="flex flex-1 items-start justify-between gap-4 text-left">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-sm font-bold text-slate-500">Add, edit and manage active or inactive products. Changes sync across the customer, kitchen and business views.</p>
          </div>
...
function StaffItemCard({ item, available, editing, activeTab, setActiveTab, setItemAvailability, updateItemSettings, toggleEditing, handleImageUpload, setItemInactive }: StaffItemCardProps) {
  return (
    <article className="overflow-hidden rounded-3xl bg-slate-50 p-3 ring-1 ring-slate-200">
      ...
      {editing && <ProductEditPanel item={item} activeTab={activeTab} setActiveTab={setActiveTab} updateItemSettings={updateItemSettings} handleImageUpload={handleImageUpload} setItemInactive={setItemInactive} />}
    </article>
  );
}

function ProductEditPanel({ item, activeTab, setActiveTab, updateItemSettings, handleImageUpload, setItemInactive }: Pick<StaffItemCardProps, "item" | "activeTab" | "setActiveTab" | "updateItemSettings" | "handleImageUpload" | "setItemInactive">) {
  return (
