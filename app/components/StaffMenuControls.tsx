...
function ProductEditPanel({ item, activeTab, setActiveTab, updateItemSettings, handleImageUpload, removeFromMenu }: Pick<StaffItemCardProps, "item" | "activeTab" | "setActiveTab" | "updateItemSettings" | "handleImageUpload" | "removeFromMenu">) {
  return (
    <div className="mt-3 rounded-2xl bg-white p-3 ring-1 ring-slate-200">
      ...
      {activeTab === "info" && <div className="mt-3 space-y-3"><label className="block"><span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Product name</span><input value={item.name} onChange={(event) => updateItemSettings(item.id, { name: event.target.value })} className="mt-2 w-full min-w-0 rounded-xl bg-slate-50 p-3 text-xs font-bold outline-none ring-1 ring-slate-200" /></label><div className="grid grid-cols-1 gap-2 sm:grid-cols-2"><label className="block min-w-0"><span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Category</span><select value={item.category} onChange={(event) => updateItemSettings(item.id, { category: event.target.value })} className="mt-2 w-full min-w-0 rounded-xl bg-slate-50 p-3 text-xs font-bold outline-none ring-1 ring-slate-200">{productCategories.map((category) => <option key={category} value={category}>{category}</option>)}</select></label><label className="block min-w-0"><span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Prep</span><input value={item.prep} onChange={(event) => updateItemSettings(item.id, { prep: event.target.value })} className="mt-2 w-full min-w-0 rounded-xl bg-slate-50 p-3 text-xs font-bold outline-none ring-1 ring-slate-200" /></label></div><label className="block"><span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Price</span><input type="text" inputMode="decimal" value={priceInputValue(item.price)} onChange={(event) => { const next = cleanPriceInput(event.target.value); updateItemSettings(item.id, { price: Number.parseFloat(next || "0") }); }} className="mt-2 w-full min-w-0 rounded-xl bg-slate-50 p-3 text-xs font-bold outline-none ring-1 ring-slate-200" /></label><button onClick={() => updateItemSettings(item.id, { popular: !item.popular })} className={item.popular ? "min-h-11 rounded-full bg-slate-900 px-4 py-2 text-xs font-black text-white" : "min-h-11 rounded-full bg-slate-100 px-4 py-2 text-xs font-black text-slate-600"}>Popular</button></div>}
