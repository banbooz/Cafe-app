    ...
    <main className="page-enter min-h-screen px-4 pb-32 pt-4" style={{ background: theme.background, color: theme.ink }}>
      ...
      <section className="mt-5">
        ...
        <div className="overflow-hidden rounded-[1.35rem] shadow-[0_12px_30px_rgba(29,37,40,0.06)] ring-1 ring-black/5" style={{ background: theme.panel }}>
          {visibleItems.length ? visibleItems.map((item, index) => <div key={item.id}><FoodRow item={item} qty={demoCart[item.id] || 0} theme={theme} index={index} open={() => undefined} plus={(event) => addOnly(event, item)} minus={(event) => removeOnly(event, item.id)} />{index < visibleItems.length - 1 && <div className="ml-[92px] h-px" style={{ background: `${theme.ink}16` }} />}</div>) : <p className="p-7 text-center text-sm font-bold" style={{ color: theme.muted }}>{experience.emptyText}</p>}
        </div>
      </section>
    </main>
  ...
  return <main className="min-h-screen bg-[#0f0f10] px-3 pb-28 pt-3 text-white">
    <div className="mx-auto max-w-[430px]">
      ...
      <label className="mt-4 flex h-11 items-center gap-3 rounded-full bg-[#1c1c1f] px-4 ring-1 ring-white/10">
        ...
        <input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-xs font-bold text-white outline-none placeholder:text-white/40" placeholder="Search your favorite food" />
        <button type="button" onClick={openMenu} className="text-white/55">☷</button>
      </label>

      <button onClick={(event) => add(event, hero)} className="mt-4 grid h-[148px] w-full grid-cols-[1.05fr_0.95fr] overflow-hidden rounded-[1.6rem] bg-[#f5c116] text-left text-black shadow-[0_18px_40px_rgba(245,193,22,0.2)] ring-4 ring-white">
        <div className="p-4">
        ...
        <div className="relative">
          <div className="absolute -right-2 bottom-2 h-[118px] w-[138px] rotate-[-8deg] rounded-[1.8rem] bg-white bg-cover bg-center shadow-[0_18px_30px_rgba(0,0,0,0.18)]" style={{ backgroundImage: `url(${hero.image})` }} />
          <div className="absolute right-20 top-4 h-10 w-10 rounded-full border-4 border-[#e83c2d]" />
          <div className="absolute right-8 top-10 h-9 w-9 rounded-full border-4 border-[#e83c2d]" />
        </div>
      </button>

      <section className="mt-5 rounded-t-[2rem] bg-[#f7f7f5] px-3 pb-7 pt-4 text-[#121516] shadow-[0_-20px_40px_rgba(0,0,0,0.22)]">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-[15px] font-black">Food Categories</h2>
          <button onClick={() => { setCategory("All"); if (popularOnly) showAll(); }} className="text-[11px] font-black text-[#e3aa09]">See All</button>
        </div>
        <div className="grid grid-cols-5 gap-2">
          {categoryTiles.map((tile) => {
            const active = category === tile.value && !popularOnly;
            return <button key={tile.value} onClick={() => { setCategory(tile.value); if (popularOnly) showAll(); }} className="rounded-[1.05rem] px-1 py-2 text-center shadow-sm ring-1 ring-black/5" style={{ background: active ? "#f5c116" : "white" }}><span className="mx-auto grid h-9 w-9 place-items-center rounded-xl bg-[#f5f1e7] text-lg">{tile.icon}</span><span className="mt-1 block text-[9px] font-black leading-tight">{tile.label}</span></button>;
          })}
        </div>

        <div className="mt-5 flex items-center justify-between">
          <h2 className="text-[15px] font-black">Popular Food</h2>
          <button onClick={() => { setCategory("All"); showAll(); }} className="text-[11px] font-black text-[#e3aa09]">See All</button>
        </div>
        <div className="mt-3 grid gap-3">
          {visible.slice(0, 8).map((item, index) => <RestaurantFoodCard key={item.id} item={item} qty={cart[item.id] || 0} index={index} open={() => openItem(item)} add={add} remove={remove} />)}
        </div>
      </section>
    </div>
    ...
    <nav className="fixed bottom-0 left-1/2 z-50 w-full max-w-[430px] -translate-x-1/2 px-4 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
      <div className="grid grid-cols-4 items-center rounded-[1.35rem] bg-[#111113]/95 p-2 shadow-[0_20px_48px_rgba(0,0,0,0.45)] ring-1 ring-white/10 backdrop-blur-xl">
        <button onClick={() => { setCategory("All"); if (popularOnly) showAll(); }} className="rounded-2xl bg-[#f5c116] py-2 text-[11px] font-black text-black">Home</button>
        <button onClick={() => setCategory("All")} className="py-2 text-[11px] font-black text-white/70">Menu</button>
  ...
  return <article onClick={open} className="flex min-h-[104px] items-center gap-3 rounded-[1.45rem] bg-white p-2.5 shadow-[0_12px_30px_rgba(17,21,23,0.08)] ring-1 ring-black/5" style={{ animationDelay: `${Math.min(index, 8) * 35}ms` }}>
    ...
    <div className="min-w-0 flex-1">
      <div className="flex items-start justify-between gap-2">
        <span className="rounded-md bg-[#f5efd9] px-2 py-1 text-[8px] font-black text-[#8a6a11]">{item.category}</span>
        <span className="text-[10px] font-black text-[#e0a400]">★ {item.popular ? "4.6" : "3.8"}</span>
      </div>
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        <span className="text-[12px] font-black text-[#101316]">{money(item.price)}</span>
        {qty ? <div className="flex items-center rounded-full bg-[#f2f2ef] p-0.5"><button onClick={(event) => remove(event, item.id)} className="grid h-6 w-6 place-items-center rounded-full bg-white text-xs font-black text-[#101316]">-</button><span className="min-w-6 text-center text-[10px] font-black">{qty}</span><button onClick={(event) => add(event, item)} className="grid h-6 w-6 place-items-center rounded-full bg-[#101316] text-xs font-black text-white">+</button></div> : <button onClick={(event) => add(event, item)} className="grid h-7 w-7 place-items-center rounded-full border border-[#101316] text-sm font-black text-[#101316]">+</button>}
      </div>
    </div>
  </article>;
}
