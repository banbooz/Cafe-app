...
type Props = {
  ...
  setTipPercentage: (tipPercentage: number | null) => void;
  close: () => void;
  add: (id: number) => void;
  ...
  return (
    <div className="sheet-backdrop-enter fixed inset-0 z-[90] flex items-end justify-center bg-[#111517]/45 p-3 backdrop-blur-sm">
      <div className="sheet-panel-enter w-full max-w-[430px] overflow-hidden rounded-[2rem] bg-[#f7f7f5] p-4 shadow-[0_26px_70px_rgba(29,37,40,0.32)] ring-1 ring-white/80">
        ...
        <section className="cart-row-enter mt-4 rounded-[1.5rem] bg-white p-4 shadow-sm ring-1 ring-black/5" style={{ animationDelay: "145ms" }}>
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-sm font-black text-[#1d2528]">Add tip</p>
              <p className="text-xs font-bold text-[#617174]">Optional, based on subtotal</p>
            </div>
            {tipPercentage && <button onClick={() => setTipPercentage(null)} className="rounded-full bg-[#f1f4f4] px-3 py-2 text-[11px] font-black text-[#617174]">Remove</button>}
          </div>
          <div className="mt-3 grid grid-cols-3 gap-2">
            {tipOptions.map((option) => <button key={option} onClick={() => toggleTip(option)} className={tipPercentage === option ? "min-h-10 rounded-full bg-[#263238] text-xs font-black text-white" : "min-h-10 rounded-full bg-[#f1f4f4] text-xs font-black text-[#263238] ring-1 ring-black/5"}>{option}%</button>)}
          </div>
        </section>

        <div className="cart-row-enter mt-4 rounded-[1.5rem] bg-white p-4 shadow-sm ring-1 ring-black/5" style={{ animationDelay: "170ms" }}>
          <div className="flex items-center justify-between text-sm font-bold text-[#617174]"><span>Subtotal</span><span>{money(subtotal)}</span></div>
          <div className="mt-2 flex items-center justify-between text-sm font-bold text-[#617174]"><span>Tip{tipPercentage ? ` (${tipPercentage}%)` : ""}</span><span>{money(tipAmount)}</span></div>
...
function CartRow({ item, index, add, remove }: { item: CartItem; index: number; add: (id: number) => void; remove: (id: number) => void }) {
  const unavailable = item.available === false;

  return (
    <article style={{ animationDelay: `${Math.min(index, 6) * 45}ms` }} className={unavailable ? "cart-row-enter flex items-center gap-3 rounded-[1.5rem] bg-white p-3 opacity-70 shadow-sm ring-1 ring-rose-100" : "cart-row-enter flex items-center gap-3 rounded-[1.5rem] bg-white p-3 shadow-sm ring-1 ring-black/5"}>
      ...
      <div className="flex items-center rounded-full bg-[#f1f4f4] p-1 ring-1 ring-black/5">
        ...
        <span key={item.qty} className="qty-pop min-w-8 text-center text-xs font-black">{item.qty}</span>
        <button onClick={() => add(item.id)} disabled={unavailable} className={unavailable ? "add-burst grid h-8 w-8 cursor-not-allowed place-items-center rounded-full bg-slate-200 font-black text-slate-400" : "add-burst grid h-8 w-8 place-items-center rounded-full bg-[#263238] font-black text-white shadow-sm"}>+</button>
      </div>
    </article>
  );
}
