...
const waiterThemes: Record<MenuExperienceId, { shell: string; panel: string; header: string; button: string; mutedButton: string; text: string; accentText: string; card: string }> = {
  restaurant: { shell: "bg-[#ebe6dd]", panel: "bg-[#fbfaf7]", header: "bg-[#2f2420] text-white", button: "bg-slate-950 text-white", mutedButton: "bg-[#fbfaf7] text-[#241c18] ring-1 ring-[#d8ccc0]", text: "text-[#241c18]", accentText: "text-[#c3915b]", card: "bg-[#fbfaf7] text-[#241c18] ring-[#d8ccc0]" },
  cafe: { shell: "bg-[#f6ead8]", panel: "bg-[#fffaf3]", header: "bg-[#c9843b] text-white", button: "bg-slate-950 text-white", mutedButton: "bg-[#fffaf3] text-[#2f2a24] ring-1 ring-[#eadcc4]", text: "text-[#2f2a24]", accentText: "text-[#fff1d8]", card: "bg-[#fffaf3] text-[#2f2a24] ring-[#eadcc4]" },
  drinks: { shell: "bg-[#171312]", panel: "bg-[#241c1a]", header: "bg-[#0f0b0a] text-[#fff8f0]", button: "bg-slate-950 text-white", mutedButton: "bg-[#241c1a] text-[#fff8f0] ring-1 ring-white/10", text: "text-[#fff8f0]", accentText: "text-[#d7a048]", card: "bg-[#241c1a] text-[#fff8f0] ring-white/10" },
};
  ...
  return <main className={`min-h-screen ${theme.shell} ${theme.text}`}>
    <div className={`mx-auto min-h-screen w-full max-w-6xl ${theme.panel} shadow-2xl shadow-stone-950/10`}>
      <header className={`sticky top-0 z-20 border-b border-black/10 ${theme.header} px-4 py-4 shadow-sm backdrop-blur sm:px-6`}>
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm font-bold leading-6 opacity-75">Ready orders appear here. Waiter marks orders served.</p>
          </div>
      ...
      <section className="grid gap-4 px-4 pb-8 sm:px-6 lg:grid-cols-3">
        ...
        {visibleOrders.length === 0 ? <div className={`col-span-full rounded-[1.5rem] p-8 text-center shadow-sm ring-1 ${theme.card}`}><h2 className="text-2xl font-black">No orders</h2><p className="mt-2 text-sm font-semibold opacity-65">Orders for this section will appear here.</p></div> : null}
      </section>
    </div>
  </main>;
}

function FilterButton({ active, label, count, detail, theme, onClick }: { active: boolean; label: string; count: number; detail: string; theme: { button: string; mutedButton: string }; onClick: () => void }) {
...
function OrderCard({ order, theme, canServe, markServed }: { order: KitchenOrder; theme: { button: string; card: string }; canServe: boolean; markServed: () => void }) {
