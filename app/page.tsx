...
const items: Item[] = [
  { id: 1, name: "Smashed Avocado Toast", category: "Breakfast", tag: "Sourdough, chilli, lemon", description: "Toasted sourdough with avocado, lemon, herbs and cracked pepper.", price: 6.9, image: "https://images.unsplash.com/photo-1541519227354-08fa5d50c44d?auto=format&fit=crop&w=900&q=80", allergens: ["Gluten"], prep: "7 min", popular: true, vegetarian: true },
  { id: 2, name: "Turkish Eggs", category: "Breakfast", tag: "Garlic yoghurt, paprika butter", description: "Poached eggs over whipped yoghurt with paprika butter and warm flatbread.", price: 8.4, image: "https://images.unsplash.com/photo-1525351484163-7529414344d8?auto=format&fit=crop&w=900&q=80", allergens: ["Egg", "Milk", "Gluten"], prep: "10 min", vegetarian: true },
  ...
  { id: 16, name: "Superfood Grain Bowl", category: "Salads", tag: "Avocado, grains, seeds", description: "A fresh bowl of grains, avocado, greens, seeds and lemon dressing.", price: 8.6, image: "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?auto=format&fit=crop&w=900&q=80", allergens: ["Sesame"], prep: "7 min", vegetarian: true },
  { id: 17, name: "Rosemary Fries", category: "Sides", tag: "Sea salt, rosemary", description: "Crispy fries finished with rosemary, sea salt and house dip.", price: 4.2, image: "https://images.unsplash.com/photo-1573080496219-bb080dd4f877?auto=format&fit=crop&w=900&q=80", allergens: ["None listed"], prep: "6 min", upsell: true, vegetarian: true },
  { id: 18, name: "Tomato Soup & Toast", category: "Lunch", tag: "Basil oil, toasted sourdough", description: "Smooth tomato soup with basil oil and warm toasted sourdough.", price: 6.4, image: "https://images.unsplash.com/photo-1547592166-23ac45744acd?auto=format&fit=crop&w=900&q=80", allergens: ["Gluten", "Milk"], prep: "8 min", vegetarian: true },
  ...
  { id: 21, name: "Mac & Cheese Pot", category: "Sides", tag: "Cheddar, crispy crumb", description: "Creamy macaroni cheese topped with a golden crispy crumb.", price: 5.9, image: "https://images.unsplash.com/photo-1543339494-b4cd4f7ba686?auto=format&fit=crop&w=900&q=80", allergens: ["Gluten", "Milk"], prep: "9 min", upsell: true, vegetarian: true },
  { id: 22, name: "Salted Caramel Brownie", category: "Desserts", tag: "Chocolate, caramel, sea salt", description: "Rich chocolate brownie with salted caramel and a soft centre.", price: 4.6, image: "https://images.unsplash.com/photo-1606313564200-e75d5e30476c?auto=format&fit=crop&w=900&q=80", allergens: ["Gluten", "Milk", "Egg"], prep: "4 min", upsell: true, vegetarian: true },
];
    ...
    return (
      <Shell>
        ...
        <main className="px-3 pb-28 pt-4 min-[380px]:px-4">
          <div className="relative h-56 rounded-[26px] bg-cover bg-center shadow-sm ring-1 ring-slate-200 min-[380px]:h-72" style={{ backgroundImage: `url(${selected.image})` }}>{selected.vegetarian && <VegetarianBadge className="absolute left-4 top-4" />}</div>
    ...
    return (
      <Shell>
        ...
        <main className="space-y-4 px-3 pb-40 pt-4 min-[380px]:px-4">
          ...
          {cartItems.length ? cartItems.map((item) => <article key={item.id} className="flex gap-3 rounded-3xl bg-white p-3 shadow-sm ring-1 ring-slate-200"><button onClick={() => openItem(item)} className="h-20 w-20 shrink-0 rounded-2xl bg-cover bg-center min-[380px]:h-24 min-[380px]:w-24" style={{ backgroundImage: `url(${item.image})` }} aria-label={`Open ${item.name}`} /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><h2 className="truncate font-black text-slate-950">{item.name}</h2>{item.vegetarian && <span className="shrink-0 text-[10px] font-black uppercase text-emerald-700">V</span>}</div><p className="mt-1 line-clamp-2 text-xs font-bold text-slate-500">{item.tag}</p><div className="mt-3 flex items-center justify-between gap-2"><span className="font-black">{money(item.price)}</span><Stepper qty={item.qty} minus={() => remove(item.id)} plus={() => add(item.id)} /></div></div></article>) : <Empty onClick={() => go("menu")} />}
      ...
      <div className="max-h-[88svh] w-full max-w-[430px] overflow-y-auto rounded-[2rem] bg-[#f8fafb] p-4 shadow-2xl ring-1 ring-white/30">
        ...
        <div className="mt-3 space-y-2">
          {activeItems.map((item) => {
            ...
            return <article key={item.id} className="flex gap-3 rounded-3xl bg-white p-3 shadow-sm ring-1 ring-slate-200"><div className="relative h-16 w-16 shrink-0 rounded-2xl bg-cover bg-center" style={{ backgroundImage: `url(${item.image})` }}>{item.vegetarian && <span className="absolute left-1 top-1 rounded-full bg-white/95 px-1.5 py-0.5 text-[9px] font-black uppercase text-emerald-700 shadow-sm ring-1 ring-emerald-100">V</span>}</div><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-black text-slate-950">{item.name}</h3><p className="mt-1 line-clamp-1 text-xs font-bold text-slate-500">{item.tag}</p><div className="mt-2 flex items-center justify-between gap-2"><span className="font-black">{money(item.price)}</span>{qty ? <Stepper qty={qty} minus={() => remove(item.id)} plus={() => add(item.id)} /> : <button onClick={() => add(item.id)} className="add">+</button>}</div></div></article>;
          })}
        </div>
